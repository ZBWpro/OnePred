...
def validate_config(
    ...
    # Use real batch size (after GRPO rollout_n repeat) for validation,
    # since the actor receives the repeated (and potentially expanded) batch.
    real_bsz_for_actor = config.data.train_batch_size * config.actor_rollout_ref.rollout.n
    actor_config.validate(n_gpus, real_bsz_for_actor, config.actor_rollout_ref.model)
