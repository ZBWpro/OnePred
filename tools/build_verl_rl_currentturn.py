"""
Build Current Turn RL data: convert existing agentic parquet to last-turn-only prediction format.

Data source: data/rl_agentic_train.parquet and data/rl_agentic_val.parquet
  - Only extract the last turn of conversation (query + response + feedback)
  - User profile is extracted from the system prompt
  - ground_truth reuses reward_model.ground_truth

Difference from fullhistory version:
  - prompt contains only the last turn (no history)
  - Used to verify the "less information -> worse prediction" hypothesis
...
# ============================================================
# System prompt is now imported from locale (same as fullhistory)
# ============================================================
...
# ============================================================
# Extract information from agentic parquet rows (reusing build_verl_rl_fullhistory logic)
# ============================================================
...
def format_turn_text(turn: dict, max_response_chars: int | None = None) -> str:
    """Format a single turn, truncating response to max_response_chars."""
...
def convert_row(row: dict) -> dict:
    """Convert an agentic parquet row to Current Turn single-turn format (last turn only)."""
...
def main():
    parser = argparse.ArgumentParser(description="Build Current Turn RL data from agentic parquet")
    parser.add_argument(
        "--train_input",
        type=str,
    ...
    import tools.build_verl_rl_currentturn as _self
    ...
    for split, input_path in [("train", args.train_input), ("val", args.val_input)]:
        ...
        output_path = os.path.join(args.output_dir, f"rl_currentturn_{split}.parquet")
